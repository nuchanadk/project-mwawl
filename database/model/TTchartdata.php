...
class TTchartdata
{
	...
	// GET reportgauge
	public function getDatareport($dateS,$dateE,$type,$yyear){
		
		if($type == "day")
		{
			$sqlQuery = " SELECT CONCAT(CAST(YEAR(r.dataDatetime) AS CHAR),'-',CAST(MONTH(r.dataDatetime) AS CHAR),'-',CAST(DAY(r.dataDatetime) AS CHAR)) AS datadate 
			...
			where  r.dataDatetime between '$dateS' and '$dateE'
			GROUP BY YEAR(r.dataDatetime),MONTH(r.dataDatetime),DAY(r.dataDatetime) ORDER BY DATE( r.dataDatetime)";
		}
		else
		{
			...
			where  YEAR(r.dataDatetime) = YEAR('$yyear')
